const Err = require( __dirname + '/error.js' );
...
function Cursor( connection, token, options, type ) {
    ...
    if ( ( type === 'feed' ) || ( type === 'atomFeed' ) ) {
        this.toArray = () => {
            console.error( 'toArray is unsupported for types: feed, atomFeed' );
            return [];
        };
    }
...
Cursor.prototype._next = function( callback = error => error && console.error( error ) ) {
    var self = this;
    if ( self._closed === true ) {
        return Promise.reject( new Err.ReqlDriverError(
            'You cannot call `next` on a closed ' + self._type ).setOperational() ).then( result => {
            callback( null, result );
        } ).catch( callback );
    }
    else if ( ( self._data.length === 0 ) && ( self._canFetch === false ) ) {
        return Promise.reject( new Err.ReqlDriverError(
            'No more rows in the ' + self._type.toLowerCase() ).setOperational() ).then( result => {
            callback( null, result );
        } ).catch( callback );
    }
    else {
        if ( ( self._data.length > 0 ) && ( self._data[ 0 ].length > self._index ) ) {
            ...
            if ( result instanceof Error ) {
                return Promise.reject( result ).then( result => {
                    callback( null, result );
                } ).catch( callback );
            }
            else {
...
Cursor.prototype.toArray = function( callback = error => error && console.error( error ) ) {
    var self = this;
...
Cursor.prototype.close = function( callback = error => error && console.error( error ) ) {
    var self = this;
    if ( self._closed === true ) {
        return self._closingPromise.then( result => {
            callback( null, result );
        } ).catch( callback );
    }
    self._closed = true;
        ...
        if ( keepGoing === false ) {
            ...
            if ( self._closed === false ) {
                if ( self._stackSize <= MAX_CALL_STACK ) {
                    self._next().then( resolve ).catch( function( error ) {
