...
Cursor.prototype._next = function() {
    if ( this._closed === true ) {
        return Promise.reject( new Err.ReqlDriverError( 'You cannot call `next` on a closed ' + this._type ).setOperational() );
    }
    else if ( ( this._data.length === 0 ) && ( this._canFetch === false ) ) {
        return Promise.reject( new Err.ReqlDriverError( 'No more rows in the ' + this._type.toLowerCase() ).setOperational() );
    }
    else {
...
Cursor.prototype.close = function() {
    ...
    this._closingPromise = new Promise( ( resolve, reject ) => {
        if ( ( this._canFetch === false ) && ( this._fetching === false ) ) {
            ...
            this.connection._end( this.token, endCallback, reject );
        }
    } );
