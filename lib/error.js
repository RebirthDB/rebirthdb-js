...
function ReqlDriverError( message, query, secondMessage ) {
    ...
    if ( ( Array.isArray( query ) && ( query.length > 0 ) ) || ( !Array.isArray( query ) && query != null ) ) {
        ...
        var backtrace = generateBacktrace( query, 0, null, [], {
            indent: 0,
            extra: 0
        } );
...
function ReqlRuntimeError( message, query, frames ) {
    ...
    if ( ( query != null ) && ( frames ) ) {
        ...
        var backtrace = generateBacktrace( query, 0, null, frames, {
            indent: 0,
            extra: 0
        } );
...
var _specialType = {
    ...
    VAR: function( term, index, father, frames ) {
        var result = {
            str: '',
            car: ''
        }
        ...
        var underline = Array.isArray( frames ) && ( frames.length === 0 );
        if ( Array.isArray( frames ) ) frames.shift();
    ...
    IMPLICIT_VAR: function( term, index, father, frames ) {
        var result = {
            str: '',
            car: ''
        }
...
function makeOptargs( term, index, father, frames, options ) {
    var result = {
        str: '',
        car: ''
    }
