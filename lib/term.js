const protodef = require( __dirname + '/protodef.js' );
...
const ReadableStream = require( __dirname + '/stream.js' );
const WritableStream = require( __dirname + '/writable_stream.js' );
const TransformStream = require( __dirname + '/transform_stream.js' );
...
Term.prototype.run = function( _connection, _options ) {
    const connection = _connection instanceof Connection ? _connection : {};
    const options = _options || ( !( _connection instanceof Connection ) && _connection ) || {};

    if ( this._error != null ) {
        return Promise.reject( new Error.ReqlRuntimeError( this._error, this._query, {
            b: this._frames
        } ) );
    }
    ...
    if ( helper.isPlainObject( connection ) && ( typeof connection._isConnection === 'function' ) && ( connection._isConnection() === true ) ) {
        ...
        return new Promise( ( resolve, reject ) => {
            const token = connection._getToken();

            const query = [ protodef.Query.QueryType.START ];
            query.push( this._query );

            const query_options = {};
            let send_options = false;
            if ( connection.db != null ) {
                send_options = true;
                query_options.db = this._r.db( connection.db )._query;
            }

            if ( this._r.arrayLimit != null ) {
                send_options = true;
                query_options[ this._translateArgs[ 'arrayLimit' ] ] = this._r.arrayLimit;
            };
            ...
            helper.loopKeys( options, ( options, key ) => {
                if ( keepGoing === true ) {
                    if ( ( key === 'readMode' ) || ( key === 'durability' ) || ( key === 'db' ) ||
                        ...
                        send_options = true;
                        if ( key === 'db' ) {
                            query_options[ key ] = this._r.db( options[ key ] )._query;
                        }
                        else if ( this._translateArgs.hasOwnProperty( key ) ) {
                            query_options[ this._translateArgs[ key ] ] = new Term( this._r ).expr( options[ key ] )._query;
                        }
                        else {
                            query_options[ key ] = new Term( this._r ).expr( options[ key ] )._query;
                        }
                    }
            ...
            if ( send_options === true ) {
                query.push( query_options );
            }
            connection._send( query, token, resolve, reject, this._query, options );
        } );
    }
    else {
        const poolMaster = this._r.getPoolMaster(); // if self._r is defined, so is self._r.getPool()
        if ( !poolMaster ) {
...
Term.prototype.upcase = function() {
    if ( this._fastArity( arguments.length, 0 ) === false ) {
        var _len = arguments.length;
        var _args = new Array( _len );
        for ( var _i = 0; _i < _len; _i++ ) {
...
Term.prototype.downcase = function() {
    if ( this._fastArity( arguments.length, 0 ) === false ) {
        var _len = arguments.length;
        var _args = new Array( _len );
        for ( var _i = 0; _i < _len; _i++ ) {
...
Term.prototype.error = Term.prototype.catch; // legacy
...
Term.prototype._wrap = function() {
    if ( helper.hasImplicit( this._query ) ) {
        ...
        return new Term( this._r ).expr( unused => {
            return this || unused;
        } )
    }
    else {
...
function translateOptions( options ) {
    const translated = {};
    helper.loopKeys( options, ( options, key ) => {
        const key_server = Term.prototype._translateArgs[ key ] || key;
        translated[ key_server ] = options[ key ];
    } );
    return translated;
}
