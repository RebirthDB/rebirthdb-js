...
var PING_VALUE = '__rebirthdb-js_ping__'
...
Connection.prototype.reconnect = function( options, callback = error => error && console.error( error ) ) {
    var self = this
    ...
    return new Promise( function( resolve, reject ) {
        self.close( options ).then( function() {
            self.r.connect( self.options ).then( resolve ).catch( reject )
        } ).catch( reject )
    } ).then( result => {
...
Connection.prototype.server = function( callback = error => error && console.error( error ) ) {
    var self = this
    return new Promise( function( resolve, reject ) {
...
Connection.prototype.close = function( options, callback = error => error && console.error( error ) ) {
    if ( typeof options === 'function' ) {
        callback = options
        options = {}
    }
...
Connection.prototype.noreplyWait = function( callback = error => error && console.error( error ) ) {
    var self = this
