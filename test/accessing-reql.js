...
describe( 'accessing-reql', function() {
    ...
    it( '`noreplyWait` should work', async function() {
        dbName = uuid()
        tableName = uuid()
        const largeishObject = Array( 10000 ).fill( Math.random() ).map( ( random ) => r.expr( {
            random
        } ) )
        ...
        let result = await r.db( dbName ).table( tableName ).insert( largeishObject ).run( connection, {
            noreply: true
        } )
    ...
    it( '`binaryFormat` should work', async function() {
        const result = await r.binary( Buffer.from( [ 1, 2, 3 ] ) ).run( connection, {
            binaryFormat: 'raw'
        } )
    ...
    it( '`profile` should work', async function() {
        let result = await r.expr( true ).run( connection, {
            profile: false
        } )
        assert( result )

        result = await r.expr( true ).run( connection, {
            profile: true
        } )
        assert( result.profile )
